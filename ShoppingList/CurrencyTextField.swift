...
@IBDesignable class CurrencyTextField : UITextField{
    ...
    override func willMoveToSuperview(newSuperview: UIView!) {
        if newSuperview != nil {
            NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(textDidChange(_:)), name:UITextFieldTextDidChangeNotification, object: self)
        } else {
    ...
    func setAmount (amount : Double){
        let textFieldStringValue = currencyFormatter.stringFromNumber(amount)
